...
Do you have an ID range in the idranges file (to-idranges.owl), in
 ...
 * https://github.com/cmungall/plant-trait-ontology
...
Email google group at INSERT EDITORS LIST HERE to lock the files  
Example  “[LOCKING] to-edit..owl for editing”

   git pull

Then, open the file to-edit.owl in Protege
...
Commit your changes
   ...
   git commit -m "COMMIT MESSAGE" to-edit.owl

If you are fixing as issue in the tracker, always reference this with
a '#'. E.g.

   git commit -m "Fixed definition of hirsutism. Fixes issue #3" to-edit.owl
...
Example session from view of command line:
  ...
  # [edit in Protege]
  # [save in Protege]
  git diff to-edit.owl
  git commit -m "polished up skull" to-edit.owl
  git pull
  git push
...
You can check on the build here:
  http://build.berkeleybop.org/job/build-to/
  
Check for errors in the report, send an email to curators if you cannot determine what the error is.

MIREOTING
---------
Sometimes you may wish to reference a class from another ontology in the context of editing TO, and the term may not yet be mireoted. You can currently pull in a new term from GO, Uberon, Chebi, CL, PATO or PR. 
...
2. Whilst editing TO, change the "Active Ontology" file in the top header to the import file that will house the new class, for example, uberon_import.owl
